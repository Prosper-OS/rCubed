...
{
    private static var c_rankComplete                    : Dynamic;
    private static var c_rankFail                    : Dynamic;
    public static inline var MENU_SONGSELECTION                       : Dynamic= "MenuSongSelection";
    public static inline var MENU_MULTIPLAYER                       : Dynamic= "MenuMultiplayer";
    public static inline var MENU_TOKENS                       : Dynamic= "MenuTokens";
    public static inline var MENU_FILTERS                       : Dynamic= "MenuFilter";
    public static inline var MENU_REPLAYS                       : Dynamic= "MenuReplays";
    public static inline var MENU_LOCAL                       : Dynamic= "MenuLocal";
    public static inline var MENU_OPTIONS                       : Dynamic= "MenuOptions";
    
    private var _gvars                       : Dynamic= GlobalVariables.instance;
    private var _lang                       : Dynamic= Language.instance;
    
    public var _MenuSingleplayer                       : Dynamic;
    private static var _MenuMultiplayer                       : Dynamic;
    private var _MenuTokens                       : Dynamic;
    
    private var hover_message                       : Dynamic;
    private var user_text                       : Dynamic;
    private var menuItemBox                       : Dynamic;
    private var logo                       : Dynamic;
    
    public var menuMusicControls                       : Dynamic;
    private var mmc_icons                     : Dynamic= [];
    private var mmc_functions                     : Dynamic= [];
    private var mmc_buttons                       : Dynamic= [];
    private var mmc_strings(default, never)                       : Dynamic= ["play", "pause", "stop", "remove"];
    
    private var statUpdaterBtn                       : Dynamic;
    private var rankUpdateThrobber                       : Dynamic;
    
    public var menuItems                       : Dynamic= [["menu_play", MENU_SONGSELECTION, false, "iconPlay"], 
    ...
    public var activePanelID                       : Dynamic= -1;
    
    ///- Constructor
    public function new(myParent                       : Dynamic)
    {
        super(myParent);
        mmc_icons = [new IconPlay(), new IconPause(), new IconStop(), new IconDelete()];
        mmc_functions = [playMusic, pauseMusic, stopMusic, deleteMusic];
    ...
    //- Add Logo
    {
        ...
        //- Add Menu Background
        var menu_bg                       : Dynamic= new MainMenuBackground();
        menu_bg.x = 145;
        ...
        {
            var menu_music_button                       : Dynamic= new BoxIcon(null, 5 + 30 * i, 5, 25, 25, mmc_icons[i], mmc_functions[i]);
        ...
        if (as3hx.Compat.truthy(_gvars.menuMusic))
        {
            ...
            if (as3hx.Compat.truthy(!_gvars.menuMusic.isPlaying && !_gvars.menuMusic.userStopped))
            {
        ...
        if (as3hx.Compat.truthy(as3hx.Compat.field(Flags.VALUES, Flags.MP_MENU_RETURN) != null))
        {
            switchTo(MENU_MULTIPLAYER);
            Reflect.setField(Flags.VALUES, Flags.MP_MENU_RETURN, false);
        }
        else
        {
    ...
    {
        if (as3hx.Compat.truthy(_MenuSingleplayer != null))
        {
            _MenuSingleplayer.stageRemove();
        }
        
        if (as3hx.Compat.truthy(_MenuTokens != null))
        {
            _MenuTokens.stageRemove();
        }
        
        if (as3hx.Compat.truthy(_MenuMultiplayer != null))
        {
    ...
    override public function dispose() : Void
    {
        if (as3hx.Compat.truthy(_MenuSingleplayer != null))
        {
            ...
            if (as3hx.Compat.truthy(this.contains(_MenuSingleplayer)))
            {
        ...
        {
            _MenuTokens.dispose();
            if (as3hx.Compat.truthy(this.contains(_MenuTokens)))
            {
        ...
        if (as3hx.Compat.truthy(_MenuMultiplayer != null))
        {
            if (as3hx.Compat.truthy(this.contains(_MenuMultiplayer)))
            {
    ...
    {
        if (as3hx.Compat.truthy(menuItemBox != null))
        {
        ...
        //- Add Menu Buttons
        var i                       : Dynamic= null;
        var btnLarge                       : Dynamic= menuItems.length;
        
        for (i in 0...menuItems.length)
        {
            if (as3hx.Compat.truthy(menuItems[i][2] != null))
            {
                btnLarge--;
            }
        }
        
        var btnWidth                       : Dynamic= as3hx.Compat.parseInt((604 - ((menuItems.length - btnLarge) * 28) - (6 * (menuItems.length - 1))) / btnLarge);
        var btnPosition                       : Dynamic= 0;
        
        for (i in 0...menuItems.length)
        {
            if (as3hx.Compat.truthy(menuItems[i][2] != null))
            {
                var menuItemSmall                       : Dynamic= new BoxIcon(menuItemBox, btnPosition, 0, 28, 28, IconUtil.getIcon(menuItems[i][3]), menuItemClick);
                ...
                if (as3hx.Compat.truthy(menuItems[i][1] == MENU_FILTERS && _gvars.activeFilter != null))
                {
            ...
            {
                var menuItem                       : Dynamic= new MenuButton(menuItemBox, btnPosition, btnWidth, _lang.string(menuItems[i][0]), i == activePanelID, menuItemClick);
    ...
    {
        if (as3hx.Compat.truthy(menuMusicControls == null))
        {
        ...
        if (as3hx.Compat.truthy(!contains(menuMusicControls)))
        {
    ...
    // Context Menu Display song Playing
    {
        ...
        var musicContextMenu                       : Dynamic= new ContextMenu();
        var musicContextMenuPlaying                       : Dynamic= new ContextMenuItem(sprintf(_lang.stringSimple("main_menu_now_playing"), {
    ...
    private function playMusic(e                       : Dynamic) : Void
    {
    ...
    private function pauseMusic(e                       : Dynamic) : Void
    {
        if (as3hx.Compat.truthy(_gvars.menuMusic && _gvars.menuMusic.isPlaying))
        {
    ...
    private function stopMusic(e                       : Dynamic) : Void
    {
        if (as3hx.Compat.truthy(_gvars.menuMusic && _gvars.menuMusic.isPlaying))
        {
    ...
    private function deleteMusic(e                       : Dynamic) : Void
    {
        if (as3hx.Compat.truthy(_gvars.menuMusic))
        {
            _gvars.menuMusic.userStop();
    ...
    //- Check Parent Function first.
    {
        ...
        var doStageAddAnyway                       : Dynamic= false;
        
        if (as3hx.Compat.truthy(_panel == MENU_OPTIONS))
        {
            addPopup(Main.POPUP_OPTIONS);
            return true;
        }
        else if (as3hx.Compat.truthy(_panel == MENU_FILTERS))
        {
            addPopup(new PopupFilterManager(this));
            return true;
        }
        else if (as3hx.Compat.truthy(_panel == MENU_REPLAYS))
        {
            addPopup(new ReplayHistoryWindow(this));
            return true;
        }
        else if (as3hx.Compat.truthy(_panel == MENU_LOCAL))
        {
        ...
        if (as3hx.Compat.truthy(initValid || doStageAddAnyway))
        {
            panel.stageAdd();
        }
    ...
    private function menuItemClick(e                       : Dynamic= null) : Void
    {
        switchTo(e.target.panel);
    }
    
    private function e_statUpdaterMouseOver(e                       : Dynamic) : Void
    {
    ...
    private function e_statUpdaterMouseOut(e                       : Dynamic) : Void
    {
        statUpdaterBtn.removeEventListener(MouseEvent.MOUSE_OUT, e_statUpdaterMouseOut);
        removeChild(hover_message);
    }
    
    private function displayToolTip(tx                       : Dynamic, ty                       : Dynamic, text                       : Dynamic, align                       : Dynamic= "center") : Void
    {
    ...
    private function e_statUpdaterClick(e                       : Dynamic) : Void
    {
        if (as3hx.Compat.truthy(rankUpdateThrobber == null))
        {
        ...
        if (as3hx.Compat.truthy(rankUpdateThrobber.running))
        {
            return;
        }
        
        var wr                       : Dynamic= new WebRequest(URLs.resolve(URLs.USER_RANKS_UPDATE_URL), c_rankComplete, c_rankFail);
        wr.load({
        ...
        c_rankComplete = function(e                       : Dynamic= null) : Void
        {
        ...
        c_rankFail = function(e                       : Dynamic) : Void
        {
