...
class GraphAccuracy extends GraphBase
{
    ...
    // Make sure hovering over graph.
    {
        ...
        for (i in minCheckNote...maxCheckNote + 1) {
dx = cross_points[i].x - mx;
            dy = cross_points[i].y - my;
            distance = Math.sqrt(dx * dx + dy * dy);
            if (distance <= minDistance)
            {
                minDistance = as3hx.Compat.parseInt(distance);
                nearest_cross = i;
            }
        }
...
dx = boo_points[i].x - mx;
        ...
        var nearest_cross_index : Int = as3hx.Compat.parseInt(nearest_cross + ((nearest_boo) ? 1000000 : 0));  // Give boo a shift to prevent potential index overlaps.  
    ...
    private function generateGraph() : Void
    {
        ...
        var pos_x : Float;
        var pos_y : Float;
        var ratio_x : Float = graphWidth / Math.max(1, song_arrows - 1);
