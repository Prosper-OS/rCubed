...
class BlitMask extends Sprite
{
    ...
    /**
		 ...
		 */
    public function new(target : DisplayObject, x : Float = 0, y : Float = 0, width : Float = 100, height : Float = 100, smoothing : Bool = false, autoUpdate : Bool = false, fillColor : Int = 0x00000000, wrap : Bool = false)
    {
        super();
        if (width < 0 || height < 0)
        {
...
if (x + _width > _bounds.right)
    ...
    {
        if (this.mouseEnabled && (!_bitmapMode || (Std.is(event, MouseEvent) && this.hitTestPoint(cast((event), MouseEvent).stageX, cast((event), MouseEvent).stageY, false))))
        {
            dispatchEvent(event);
        }
    }
