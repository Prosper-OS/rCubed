...
class RoughEase extends Ease
{
    ...
    /** @private **/
    private static var _lookup : Dynamic = { };  //keeps track of all named instances so we can find them in byName().  
    /** @private **/
    ...
    /**
		 ...
		 * </ul>
		 */
    public function new(vars : Dynamic = null, args : Array<Dynamic> = null)
    {
        super();
        if (as3hx.Compat.typeof((vars)) != "object" || vars == null)
        {
            vars = {
                        strength : vars,
                        points : args[0],
        ...
        {
            _name = vars.name;
            Reflect.setField(_lookup, Std.string(vars.name), this);
        }
        else
        {
        ...
        var a : Array<Dynamic> = [];
        var cnt : Int = 0;
        var points : Int = as3hx.Compat.parseInt(vars.points) || 20;
        var i : Int = points;
        var randomize : Bool = (vars.randomize != false);
        var clamp : Bool = (vars.clamp == true);
        var template : Ease = ((Std.is(vars.template, Ease))) ? vars.template : null;
        var strength : Float = ((as3hx.Compat.typeof((vars.strength)) == "number")) ? vars.strength * 0.4 : 0.4;
        var x : Float;
        var y : Float;
        var bump : Float;
        var invX : Float;
...
invX = x * 2;
    ...
    /**
		 ...
		 */
    public static function create(strength : Float = 1, points : Int = 20, clamp : Bool = false, templateEase : Ease = null, taper : String = "none", randomize : Bool = true, name : String = "") : Ease
    {
    ...
    {
        return Reflect.field(_lookup, name);
    }
    ...
    {
        Reflect.deleteField(_lookup, _name);
    }
    ...
    private function set_name(value : String) : String
    {
        Reflect.deleteField(_lookup, _name);
        _name = value;
        Reflect.setField(_lookup, _name, this);
        return value;
    }
