...
class TimelineMax extends TimelineLite implements IEventDispatcher
{
    ...
    public function tweenTo(position : Dynamic, vars : Dynamic = null) : TweenLite
    {
        ...
        var duration : Float = (Math.abs(as3hx.Compat.parseFloat(copy.time) - _time) / _timeScale) || 0.001;
...
tl.add( myTimeline.tweenFromTo("myLabel2", 0) );
    ...
    public function tweenFromTo(fromPosition : Dynamic, toPosition : Dynamic, vars : Dynamic = null) : TweenLite
    {
        ...
        return try cast(t.duration((Math.abs(t.vars.time - fromPosition) / _timeScale) || 0.001), TweenLite) catch(e:Dynamic) null;
    }
        ...
        if (_cycle != prevCycle)
        {
            ...
            /*
				...
				*/{
                ...
                var wrap : Bool = (backwards == (_yoyo && (_cycle & 1) != 0));
                var recTotalTime : Float = _totalTime;
                var recCycle : Int = _cycle;
                var recRawPrevTime : Float = _rawPrevTime;
                var recTime : Float = _time;
