...
class FrameForwardPlugin extends TweenPlugin
{
    ...
    override public function _onInitTween(target : Dynamic, value : Dynamic, tween : TweenLite) : Bool
    {
        ...
        _change = ((as3hx.Compat.typeof((value)) == "number")) ? as3hx.Compat.parseFloat(value) - _start : ((as3hx.Compat.typeof((value)) == "string" && value.charAt(1) == "=")) ? as3hx.Compat.parseInt(value.charAt(0) + "1") * as3hx.Compat.parseFloat(value.substr(2)) : as3hx.Compat.parseFloat(value) || 0;
        if (!_backward && _change < 0)
        {
            _change = as3hx.Compat.parseInt(((_change + (_max * 99999)) % _max) + ((_change / _max) | 0) * _max);
        }
