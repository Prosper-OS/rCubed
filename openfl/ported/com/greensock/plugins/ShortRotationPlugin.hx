...
class ShortRotationPlugin extends TweenPlugin
{
    ...
    {
        if (as3hx.Compat.typeof((value)) == "number")
        {
            return false;
        }
        ...
        for (p in Reflect.fields(value))
        {
            if (p != "useRadians")
            {
                start = ((Std.is(Reflect.field(target, p), Dynamic))) ? Reflect.field(target, Std.string((((p.indexOf("set") || !(Lambda.has(target, "get" + p.substr(3))))) ? p : "get" + p.substr(3))))() : Reflect.field(target, p);
                _initRotation(target, p, start, ((as3hx.Compat.typeof((Reflect.field(value, p))) == "number")) ? as3hx.Compat.parseFloat(Reflect.field(value, p)) : start + as3hx.Compat.parseFloat(Reflect.field(value, p).split("=").join("")), useRadians);
            }
        }
        return true;
    }
