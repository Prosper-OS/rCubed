...
class BezierPlugin extends TweenPlugin
{
    ...
    private var _l2 : Float;  //length 2 (upper)  
    ...
    private var _s2 : Float;  //segment 2 (upper)  
    ...
    private var _prec : Float;  //precision  
    /** @private **/
    ...
    {
        this._target = target;
        var vars : Dynamic = ((Std.is(value, Array))) ? {
            values : value
        } : value;
        this._props = [];
        this._timeRes = ((vars.timeResolution == null)) ? 6 : as3hx.Compat.parseInt(vars.timeResolution);
        ...
        this._autoRotate = (autoRotate != null) ? ((Std.is(autoRotate, Array))) ? try cast(autoRotate, Array<Dynamic>) catch(e:Dynamic) null : [["x", "y", "rotation", (((autoRotate == true)) ? 0 : as3hx.Compat.parseFloat(autoRotate))]] : null;
        
        if (Std.is(second, Point))
        {
            this._props = ["x", "y"];
        }
        else
        ...
        while (--i > -1)
        {
            ...
            isFunc = this._func[p] = (Std.is(Reflect.field(target, p), Dynamic));
        ...
        if (ar != null)
        {
            ...
            while (--i > -1)
            {
                ...
                {
                    p = ar[i][j];
                    this._func[p] = ((Std.is(Reflect.field(target, p), Dynamic))) ? Reflect.field(target, Std.string((((p.indexOf("set") || !(Lambda.has(target, "get" + p.substr(3))))) ? p : "get" + p.substr(3)))) : false;
                }
                p = ar[i][2];
 ...
 for (var i:int = 0; i &lt; bx.length; i++) {
 	g.curveTo(bx[i].b, by[i].b, bx[i].c, by[i].c);
		 ...
		 */
    public static function bezierThrough(values : Array<Dynamic>, curviness : Float = 1, quadratic : Bool = false, basic : Bool = false, correlate : String = "x,y,z", prepend : Dynamic = null) : Dynamic
    {
        var obj : Dynamic = { };
...
seamless = false;
            ...
            if (seamless)
            {
                values = values.copy();  //duplicate the array to avoid contaminating the original which the user may be reusing for other tweens  
                if (prepend != null)
                {
        ...
        as3hx.Compat.setArrayLength(_r1, as3hx.Compat.setArrayLength(_r2, as3hx.Compat.setArrayLength(_r3, 0)));
        i = props.length;
        while (--i > -1)
        {
            p = props[i];
        ...
        if (!basic)
        {
            ...
            while (--i > -1)
            {
                if (Reflect.field(_corProps, p) != null)
                {
                    ...
                    l = as3hx.Compat.parseInt(a.length - 1);
                    for (j in 0...l)
                    {
        ...
        while (--i > -1)
        {
            ...
            _calculateControlPoints(a, curviness, quadratic, basic, Reflect.field(_corProps, p));  //this method requires that _parseAnchors() and _setSegmentRatios() ran first so that _r1, _r2, and _r3 values are populated for all properties  
            if (seamless)
            {
                a.splice(0, j);
    ...
    public static function _parseBezierData(values : Array<Dynamic>, type : String, prepend : Dynamic = null) : Dynamic
    {
        ...
        while (--i > -1)
        {
            ...
            for (j in 0...l)
            {
                a = ((prepend == null)) ? Reflect.field(values[j], p) : ((as3hx.Compat.typeof(tmp = Reflect.field(values[j], p)) == "string" && tmp.charAt(1) == "=")) ? Reflect.field(prepend, p) + as3hx.Compat.parseFloat(tmp.charAt(0) + tmp.substr(2)) : as3hx.Compat.parseFloat(tmp);
                if (soft)
                {
                    if (j > 1)
                    {
            ...
            l = as3hx.Compat.parseInt(cnt - inc + 1);
            cnt = 0;
            j = 0;
            while (j < l)
            {
            ...
            as3hx.Compat.setArrayLength(cur, cnt);
        }
        return obj;
    }
    ...
    private static function _parseAnchors(values : Array<Dynamic>, p : String, correlate : Bool, prepend : Dynamic) : Array<Dynamic>
    {
        ...
        if (prepend != null)
        {
            ...
            while (--i > -1)
            {
                if (as3hx.Compat.typeof(tmp = Reflect.field(values[i], p)) == "string")
                {
                    if (tmp.charAt(1) == "=")
                    {
                        Reflect.setField(values[i], p, Reflect.field(prepend, p) + as3hx.Compat.parseFloat(tmp.charAt(0) + tmp.substr(2)));
                    }
                }
            }
        }
        
        l = as3hx.Compat.parseInt(values.length - 2);
        if (l < 0)
        {
    ...
    {
        var l : Int = as3hx.Compat.parseInt(a.length - 1);
        var ii : Int = 0;
        var cp1 : Float = a[0].a;
        var i : Int;
        var p1 : Float;
        ...
        for (i in 0...l)
        {
            ...
            if (quad)
            {
                ...
                as3hx.Compat.arraySplice(a, ii, 1, [qb[0], qb[1], qb[2], qb[3]]);
                ii += 4;
            }
            else
            {
    ...
    private static function _parseLengthData(obj : Dynamic, precision : Int = 6) : Dynamic
    {
        ...
        var d : Float = 0;
        var total : Float = 0;
        var threshold : Int = as3hx.Compat.parseInt(precision - 1);
    ...
    private static function _addCubicLengths(a : Array<Dynamic>, steps : Array<Dynamic>, precision : Int = 6) : Void
    {
        ...
        while (--j > -1)
        {
            ...
            for (i in 1...precision + 1)
            {
                ...
                index = as3hx.Compat.parseInt(j * precision + i - 1);
    ...
    override public function setRatio(v : Float) : Void
    {
        ...
        else
        {
            ...
            {
                l = as3hx.Compat.parseInt(segments - 1);
            ...
            {
                l = as3hx.Compat.parseInt(curSeg.length - 1);
