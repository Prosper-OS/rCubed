...
{
    /** @private **/
    public static inline var version : String = "12.0.13";
    ...
    private var _firstPT : PropTween;
    ...
    /**
		 ...
		 */
    public function new(props : String = "", priority : Int = 0)
    {
        _overwriteProps = props.split(",");
        _propName = _overwriteProps[0];
        _priority = (priority || 0) ? 1 : 0;
    }
    ...
    /**
		 ...
		 */
    public function _onInitTween(target : Dynamic, value : Dynamic, tween : TweenLite) : Bool
    {
        return false;
    }
    ...
    /**
		 ...
		 */
    private function _addTween(target : Dynamic, propName : String, start : Float, end : Dynamic, overwriteProp : String = null, round : Bool = false) : PropTween
    {
        var c : Float = ((end == null)) ? 0 : ((as3hx.Compat.typeof((end)) == "number" || end.charAt(1) != "=")) ? as3hx.Compat.parseFloat(end) - start : as3hx.Compat.parseInt(end.charAt(0) + "1") * as3hx.Compat.parseFloat(end.substr(2));
        if (c != 0)
        {
    ...
    /**
		 ...
		 */
    public function setRatio(v : Float) : Void
    {
    ...
    /**
		 ...
		 */
    public function _roundProps(lookup : Dynamic, value : Bool = true) : Void
    {
        var pt : PropTween = _firstPT;
        while (pt)
        {
            if ((Lambda.has(lookup, _propName)) || (pt.n != null && Lambda.has(lookup, pt.n.split(_propName + "_").join("")))) {
pt.r = value;
            }
            pt = pt._next;
        }
    ...
    /**
		 ...
		 */
    public function _kill(lookup : Dynamic) : Bool
    {
        if (Lambda.has(lookup, _propName))
        {
            _overwriteProps = [];
        }
        else
        {
            ...
            while (--i > -1)
            {
                if (Lambda.has(lookup, _overwriteProps[i]))
                {
    ...
    /**
		 ...
		 */
    private static function _onTweenEvent(type : String, tween : TweenLite) : Bool
    {
...
var pt2 : PropTween;
            ...
            while (pt)
            {
                ...
                pt._prev = (pt2 != null) ? pt2._prev : last;
                if (pt._prev)
                {
                    pt._prev._next = pt;
                }
    ...
    public static function activate(plugins : Array<Dynamic>) : Bool
    {
        ...
        while (--i > -1)
        {
            ...
            {
                var pluginInstance : TweenPlugin = Type.createInstance(plugins[i], []);
                TweenLite._plugins[pluginInstance._propName] = plugins[i];
            }
        }
        return true;
    }
