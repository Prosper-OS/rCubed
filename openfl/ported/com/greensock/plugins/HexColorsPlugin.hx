...
class HexColorsPlugin extends TweenPlugin
{
    ...
    override public function _onInitTween(target : Dynamic, value : Dynamic, tween : TweenLite) : Bool
    {
        ...
        {
            _initColor(target, p, as3hx.Compat.parseInt(Reflect.field(value, p)));
        }
        return true;
    }
    ...
    public function _initColor(target : Dynamic, p : String, end : Int) : Void
    {
        ...
        var start : Int = ((!isFunc)) ? Reflect.field(target, p) : Reflect.field(target, Std.string((((p.indexOf("set") || !(Lambda.has(target, "get" + p.substr(3))))) ? p : "get" + p.substr(3))))();
        if (start != end)
        {
            var r : Int = start >> 16;
            var g : Int = as3hx.Compat.parseInt(start >> 8) & 0xff;
            var b : Int = start & 0xff;
            _colors[_colors.length] = new ColorProp(target, p, isFunc, r, (end >> 16) - r, g, (as3hx.Compat.parseInt(end >> 8) & 0xff) - g, b, (end & 0xff) - b);
