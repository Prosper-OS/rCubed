...
class DirectionalRotationPlugin extends TweenPlugin
{
    ...
    override public function _onInitTween(target : Dynamic, value : Dynamic, tween : TweenLite) : Bool
    {
        ...
        var end : Float;
        var dif : Float;
        var split : Array<Dynamic>;
        var type : String;
        for (p in Reflect.fields(value))
        {
            if (p != "useRadians")
            {
                split = (Reflect.field(value, p) + "").split("_");
                v = split[0];
                type = split[1];
                start = as3hx.Compat.parseFloat(((as3hx.Compat.typeof((Reflect.field(target, p))) != "function")) ? Reflect.field(target, p) : Reflect.field(target, Std.string((((p.indexOf("set") || as3hx.Compat.typeof((Reflect.field(target, Std.string("get" + p.substr(3))))) != "function")) ? p : "get" + p.substr(3))))());
                end = Reflect.setField(finals, p, ((as3hx.Compat.typeof((v)) == "string" && v.charAt(1) == "=")) ? start + as3hx.Compat.parseInt(v.charAt(0) + "1") * as3hx.Compat.parseFloat(v.substr(2)) : as3hx.Compat.parseFloat(v) || 0);
                dif = end - start;
                if (type == "short")
                {
                    dif = dif % cap;
    ...
    override public function setRatio(v : Float) : Void
    {
        ...
        else
        {
            ...
            while (pt)
            {
                if (pt.f)
                {
                    pt.t[pt.p](Reflect.field(finals, Std.string(pt.p)));
                }
                else
                {
                    pt.t[pt.p] = Reflect.field(finals, Std.string(pt.p));
