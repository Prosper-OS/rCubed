...
var a2 = TweenMax.getTweensOf([myObject1, myObject2]); //finds 3 tweens
    ...
    override public function invalidate() : Dynamic
    {
        ...
        _repeat = (this.vars.repeat || 0) ? 1 : 0;
        _repeatDelay = this.vars.repeatDelay || 0;
...
tween.updateTo({x:300, y:0}, false);
    ...
    public function updateTo(vars : Dynamic, resetDuration : Bool = false) : Dynamic
    {
        ...
        {
            this.vars[p] = Reflect.field(vars, p);
        }
        if (_initted)
        {
            if (resetDuration)
...
force = true;
        ...
        else
        {
            ...
            if (_repeat != 0)
            {
                ...
                _cycle = as3hx.Compat.parseInt(_totalTime / cycleDuration) >> 0;  //originally _totalTime % cycleDuration but floating point errors caused problems, so I normalized it. (4 % 0.8 should be 0 but Flash reports it as 0.79999999!)  
                if (_cycle != 0)
                {
            ...
            if (_easeType != 0)
            {
                var r : Float = _time / _duration;
                var type : Int = _easeType;
                var pow : Int = _easePower;
...
_rawPrevTime = 0;
    ...
    private function _initDispatcher() : Bool
    {
        ...
        for (p in Reflect.fields(_listenerLookup))
        {
            ...
            {
                if (Std.is(Reflect.field(vars, p), Dynamic))
                {
                    if (_dispatcher == null)
                    {
...
TweenMax.staggerTo(textFields, 1, {y:"+150", ease:CubicIn.ease}, 0.2);
		 ...
		 * @see com.greensock.TimelineLite#staggerTo()
		 */
    public static function staggerTo(targets : Array<Dynamic>, duration : Float, vars : Dynamic, stagger : Float = 0, onCompleteAll : Dynamic = null, onCompleteAllParams : Array<Dynamic> = null) : Array<Dynamic>
    {
        vars = _prepVars(vars, false);
        var a : Array<Dynamic> = [];
        var l : Int = targets.length;
        var delay : Float = vars.delay || 0;
        var copy : Dynamic;
        var i : Int;
        var p : String;
        for (i in 0...l)
...
TweenMax.staggerFrom(textFields, 1, {y:"+150"}, 0.2);
		 ...
		 * @see com.greensock.TimelineLite#staggerFrom()
		 */
    public static function staggerFrom(targets : Array<Dynamic>, duration : Float, vars : Dynamic, stagger : Float = 0, onCompleteAll : Dynamic = null, onCompleteAllParams : Array<Dynamic> = null) : Array<Dynamic>
    {
        vars = _prepVars(vars, true);
        vars.runBackwards = true;
...
TweenMax.staggerFromTo(textFields, 1, {alpha:1}, {alpha:0}, 0.2);
		 ...
		 * @see com.greensock.TimelineLite#staggerFromTo()
		 */
    public static function staggerFromTo(targets : Array<Dynamic>, duration : Float, fromVars : Dynamic, toVars : Dynamic, stagger : Float = 0, onCompleteAll : Dynamic = null, onCompleteAllParams : Array<Dynamic> = null) : Array<Dynamic>
    {
        toVars = _prepVars(toVars, false);
        fromVars = _prepVars(fromVars, false);
        toVars.startAt = fromVars;
...
TweenMax.killAll(false, false, true, false);
    ...
    public static function killAll(complete : Bool = false, tweens : Bool = true, delayedCalls : Bool = true, timelines : Bool = true) : Void
    {
        ...
        {
            tween = a[i];
            isDC = (cast((tween), TweenLite).target == cast((tween), TweenLite).vars.onComplete);
