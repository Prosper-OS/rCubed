...
TweenNano.ticker.removeEventListener("tick", myFunction);
    ...
    public static var defaultEase : Dynamic = function(t : Float, b : Float, c : Float, d : Float) : Float
        {
            return -1 * (t /= d) * (t - 2);
        }  /** @private **/  ;
    ...
    public function new(target : Dynamic, duration : Float, vars : Dynamic)
    {
        if (_reservedProps == null)
        {
            ...
            _time = Math.round(haxe.Timer.stamp() * 1000) / 1000;
            _frame = 0;
        ...
        _ease = ((as3hx.Compat.typeof((_rawEase)) == "function")) ? try cast(_rawEase, Dynamic) catch(e:Dynamic) null : _rawEase.getRatio;
        _useFrames = cast(vars.useFrames == true, Bool);
        _startTime = ((_useFrames) ? _frame : _time) + (this.vars.delay || 0);
        
        if (this.vars.overwrite == "all" || as3hx.Compat.parseInt(this.vars.overwrite) == 1)
        {
    ...
    /** @private **/
    private function _initProps(target : Dynamic) : Void
    {
        if (target != null)
        {
            for (p in Reflect.fields(vars))
            {
                if (!(Lambda.has(_reservedProps, p)))
                {
                    _firstPT = {
                                ...
                                t : target,
                                p : p,
                                f : (as3hx.Compat.typeof((Reflect.field(target, p))) == "function")
                            };
                    _firstPT.s = ((!_firstPT.f)) ? as3hx.Compat.parseFloat(Reflect.field(target, p)) : Reflect.field(target, Std.string((((p.indexOf("set") || as3hx.Compat.typeof((Reflect.field(target, Std.string("get" + p.substr(3))))) != "function")) ? p : "get" + p.substr(3))))();
                    _firstPT.c = ((as3hx.Compat.typeof((Reflect.field(vars, p))) == "number")) ? as3hx.Compat.parseFloat(Reflect.field(vars, p)) - _firstPT.s : ((as3hx.Compat.typeof((Reflect.field(vars, p))) == "string" && Reflect.field(vars, p).charAt(1) == "=")) ? as3hx.Compat.parseInt(Reflect.field(vars, p).charAt(0) + "1") * as3hx.Compat.parseFloat(Reflect.field(vars, p).substr(2)) : as3hx.Compat.parseFloat(Reflect.field(vars, p)) || 0;
                    if (_firstPT._next)
                    {
    ...
    /**
		 ...
		 */
    public function _render(time : Float) : Void
    {
        if (!_initted)
        {
            _init();
    ...
    public function kill(target : Dynamic = null) : Void
    {
        ...
        if (Std.is(target, Array) && as3hx.Compat.typeof((Reflect.field(target, Std.string(0)))) == "object")
        {
            ...
            while (--i > -1)
            {
                kill(Reflect.field(target, Std.string(i)));
            }
            return;
        }
...
function myFunction(param1, param2) {
    ...
    {
        return new TweenNano(callback, 0, {
            delay : delay,
            onComplete : callback,
    ...
    {
        _frame += 1;
        _time = Math.round(haxe.Timer.stamp() * 1000) * 0.001;
