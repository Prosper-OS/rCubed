...
Animation.ticker.removeEventListener("tick", myFunction);
    ...
    public function new(duration : Float = 0, vars : Dynamic = null)
    {
        ...
        _delay = as3hx.Compat.parseFloat(this.vars.delay) || 0;
        _timeScale = 1;
        _totalTime = _time = 0;
        data = this.vars.data;
        _rawPrevTime = -1;
...
myAnimation.timeScale( 0.5 ); //sets timeScale to half-speed
    ...
    public function timeScale(value : Float = Math.NaN) : Dynamic
    {
        ...
        {
            var t : Float = (((_pauseTime != 0 && !Math.isNaN(_pauseTime)) || _pauseTime == 0)) ? _pauseTime : _timeline._totalTime;
 ...
 myAnimation.paused( !myAnimation.paused() ); //toggles the paused state
    ...
    public function paused(value : Bool = false) : Dynamic
    {
        ...
        if (value != _paused)
        {
            if (_timeline != null)
            {
                var raw : Float = _timeline.rawTime();
                var elapsed : Float = raw - _pauseTime;
                if (!value && _timeline.smoothChildTiming)
                {
                    _startTime += elapsed;
                    _uncache(false);
                }
                _pauseTime = (value) ? raw : Math.NaN;
                _paused = value;
