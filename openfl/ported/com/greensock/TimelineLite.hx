    ...
    public function new(vars : Dynamic = null)
    {
        ...
        for (p in Reflect.fields(this.vars))
        {
            ...
            if (Std.is(val, Array))
            {
                ...
                {
                    this.vars[p] = _swapSelfInParams(try cast(val, Array<Dynamic>) catch(e:Dynamic) null);
                }
            }
        }
...
tl.to(mc, 1, {x:100}, "myLabel+=2");  //places it 2 seconds after "myLabel"
		 ...
		 * @see #remove()
		 */
    public function to(target : Dynamic, duration : Float, vars : Dynamic, position : Dynamic = "+=0") : Dynamic
    {
        return ((duration != 0 && !Math.isNaN(duration))) ? add(new TweenLite(target, duration, vars), position) : this.set(target, vars, position);
    }
...
tl.from(mc, 1, {x:100}, "myLabel+=2");  //places it 2 seconds after "myLabel"
		 ...
		 * @see #remove()
		 */
    public function from(target : Dynamic, duration : Float, vars : Dynamic, position : Dynamic = "+=0") : Dynamic
    {
...
tl.fromTo(mc, 1, {x:0}, {x:100}, "myLabel+=2");  //places it 2 seconds after "myLabel"
		 ...
		 * @see #remove()
		 */
    public function fromTo(target : Dynamic, duration : Float, fromVars : Dynamic, toVars : Dynamic, position : Dynamic = "+=0") : Dynamic
    {
        return ((duration != 0 && !Math.isNaN(duration))) ? add(TweenLite.fromTo(target, duration, fromVars, toVars), position) : this.set(target, toVars, position);
    }
...
tl.staggerTo(myArray, 1, {x:100}, 0.25, "myLabel+=2");  //places 2 seconds after "myLabel"
		 ...
		 * @see #staggerFromTo()
		 */
    public function staggerTo(targets : Array<Dynamic>, duration : Float, vars : Dynamic, stagger : Float, position : Dynamic = "+=0", onCompleteAll : Dynamic = null, onCompleteAllParams : Array<Dynamic> = null) : Dynamic
    {
...
tl.staggerFrom(myArray, 1, {x:100}, 0.25, "myLabel+=2");  //places 2 seconds after "myLabel"
		 ...
		 * @see #staggerFromTo()
		 */
    public function staggerFrom(targets : Array<Dynamic>, duration : Float, vars : Dynamic, stagger : Float = 0, position : Dynamic = "+=0", onCompleteAll : Dynamic = null, onCompleteAllParams : Array<Dynamic> = null) : Dynamic
    {
        vars = _prepVars(vars);
        if (!(Lambda.has(vars, "immediateRender")))
        {
...
tl.staggerFromTo(myArray, 1, {x:0}, {x:100}, 0.25, "myLabel+=2");  //places 2 seconds after "myLabel"
		 ...
		 * @see #staggerFrom()
		 */
    public function staggerFromTo(targets : Array<Dynamic>, duration : Float, fromVars : Dynamic, toVars : Dynamic, stagger : Float = 0, position : Dynamic = "+=0", onCompleteAll : Dynamic = null, onCompleteAllParams : Array<Dynamic> = null) : Dynamic
    {
...
tl.set(mc, {x:100}, "myLabel+=2");  //places it 2 seconds after "myLabel"
		 ...
		 * @see #remove()
		 */
    public function set(target : Dynamic, vars : Dynamic, position : Dynamic = "+=0") : Dynamic
    {
...
timeline.addPause(4, yourFunction, ["param1", "param2"]);
    ...
    /** @private **/
    private static function _prepVars(vars : Dynamic) : Dynamic
    //to accommodate TweenLiteVars and TweenMaxVars instances for strong data typing and code hinting
    {
    ...
    /** @private **/
    private static function _copy(vars : Dynamic) : Dynamic
    {
        var copy : Dynamic = { };
        var p : String;
...
TweenLite.fromTo(myWindow, 1, {scaleX:0, scaleY:0}, {scaleX:1, scaleY:1});
		 ...
		 */
    public static function exportRoot(vars : Dynamic = null, omitDelayedCalls : Bool = true) : TimelineLite
    {
        vars = vars || { };
        if (!(Lambda.has(vars, "smoothChildTiming")))
        {
        ...
        {
            next = tween._next;
            if (!omitDelayedCalls || !(Std.is(tween, TweenLite) && cast((tween), TweenLite).target == tween.vars.onComplete))
            {
 ...
 myAnimation.insert(nested, "myLabel");
		 ...
		 * @see #add()
		 */
    override public function insert(value : Dynamic, timeOrLabel : Dynamic = 0) : Dynamic
    {
...
tl.add([tween1, tween2, tween3], "+=2", "sequence", 0.5);
    ...
    {
        if (as3hx.Compat.typeof((position)) != "number")
        {
            position = _parseTimeOrLabel(position, 0, true, value);
        }
        if (!(Std.is(value, Animation)))
        {
            if (Std.is(value, Array))
            {
                var i : Int;
                var curTime : Float = as3hx.Compat.parseFloat(position);
                var l : Float = value.length;
                var child : Dynamic;
                for (i in 0...l)
                {
                    child = Reflect.field(value, Std.string(i));
                    ...
                    add(child, curTime);
                    if (as3hx.Compat.typeof((child)) == "string" || as3hx.Compat.typeof((child)) == "function")
                    {  //do nothing  
            ...
            else if (as3hx.Compat.typeof((value)) == "string")
            {
                return addLabel(Std.string(value), position);
            }
            else if (as3hx.Compat.typeof((value)) == "function")
            {
...
var tl : SimpleTimeline = this;
                    var beforeRawTime : Bool = (tl.rawTime() > value._startTime);  //if the tween is placed on the timeline so that it starts BEFORE the current rawTime, we should align the playhead (move the timeline). This is because sometimes users will create a timeline, let it finish, and much later append a tween and expect it to run instead of jumping to its end state. While technically one could argue that it should jump to its end state, that's not what users intuitively expect.  
                    while (tl._timeline)
                    {
    ...
    public function remove(value : Dynamic) : Dynamic
    {
        ...
        {
            var i : Float = value.length;
            while (--i > -1)
            {
                remove(Reflect.field(value, Std.string(i)));
            }
            return this;
        }
        else if (as3hx.Compat.typeof((value)) == "string")
        {
            return removeLabel(Std.string(value));
        }
        return kill(null, value);
    }
    
    /** @private **/
    override public function _remove(tween : Animation, skipDisable : Bool = false) : Dynamic
    {
...
myTimeline.append(nested);
		 ...
		 * @see #set()
		 */
    public function append(value : Dynamic, offsetOrLabel : Dynamic = 0) : Dynamic
    {
    ...
    /**
		 ...
		 * @see #staggerFromTo()
		 */
    public function insertMultiple(tweens : Array<Dynamic>, timeOrLabel : Dynamic = 0, align : String = "normal", stagger : Float = 0) : Dynamic
    {
    ...
    /**
		 ...
		 */
    public function appendMultiple(tweens : Array<Dynamic>, offsetOrLabel : Dynamic = 0, align : String = "normal", stagger : Float = 0) : Dynamic
    {
    ...
    /**
		 ...
		 */
    public function addLabel(label : String, position : Dynamic = "+=0") : Dynamic
    {
        Reflect.setField(_labels, label, _parseTimeOrLabel(position));
        return this;
    }
    ...
    {
        Reflect.deleteField(_labels, label);
        return this;
    }
    ...
    {
        return ((Lambda.has(_labels, label))) ? as3hx.Compat.parseFloat(Reflect.field(_labels, label)) : -1;
    }
    
    /** @private **/
    private function _parseTimeOrLabel(timeOrLabel : Dynamic, offsetOrLabel : Dynamic = 0, appendIfAbsent : Bool = false, ignore : Dynamic = null) : Float
    {
        var i : Int;
        //if we're about to add a tween/timeline (or an array of them) that's already a child of this timeline, we should remove it first so that it doesn't contaminate the duration().
        if (Std.is(ignore, Animation) && ignore.timeline == this)
        {
            remove(ignore);
        }
        else if (Std.is(ignore, Array))
        {
            i = ignore.length;
            while (--i > -1)
            {
                if (Std.is(Reflect.field(ignore, Std.string(i)), Animation) && Reflect.field(ignore, Std.string(i)).timeline == this)
                {
                    remove(Reflect.field(ignore, Std.string(i)));
                }
            }
        }
        if (as3hx.Compat.typeof((offsetOrLabel)) == "string")
        {
            return _parseTimeOrLabel(offsetOrLabel, ((appendIfAbsent && as3hx.Compat.typeof((timeOrLabel)) == "number" && !(Lambda.has(_labels, offsetOrLabel)))) ? timeOrLabel - duration() : 0, appendIfAbsent);
        }
        offsetOrLabel = offsetOrLabel || 0;
        if (as3hx.Compat.typeof((timeOrLabel)) == "string" && (Math.isNaN(timeOrLabel) || (Lambda.has(_labels, timeOrLabel)))) {
i = timeOrLabel.indexOf("=");
            if (i == -1)
            {
                if (!(Lambda.has(_labels, timeOrLabel)))
                {
                    return (appendIfAbsent) ? (Reflect.setField(_labels, Std.string(timeOrLabel), duration() + offsetOrLabel)) : offsetOrLabel;
                }
                return Reflect.field(_labels, Std.string(timeOrLabel)) + offsetOrLabel;
            }
            offsetOrLabel = as3hx.Compat.parseInt(timeOrLabel.charAt(i - 1) + "1") * as3hx.Compat.parseFloat(timeOrLabel.substr(i + 1));
        ...
        return as3hx.Compat.parseFloat(timeOrLabel) + offsetOrLabel;
    }
...
myAnimation.seek("myLabel");
    ...
    {
        return totalTime(((as3hx.Compat.typeof((position)) == "number")) ? as3hx.Compat.parseFloat(position) : _parseTimeOrLabel(position), suppressEvents);
    }
    ...
    /**
		 ...
		 */
    public function gotoAndPlay(position : Dynamic, suppressEvents : Bool = true) : Dynamic
    {
    ...
    /**
		 ...
		 * @param suppressEvents If true, no events or callbacks will be triggered as the "virtual playhead" moves to the new position (onComplete, onUpdate, onReverseComplete, etc. of this timeline and any of its child tweens/timelines won't be triggered, nor will any of the associated events be dispatched) 
		 */
    public function gotoAndStop(position : Dynamic, suppressEvents : Bool = true) : Dynamic
    {
...
if (isComplete)
    ...
    /**
		 ...
		 */
    public function _hasPausedChild() : Bool
    {
        var tween : Animation = _first;
        while (tween)
        {
            if (tween._paused || ((Std.is(tween, TimelineLite)) && cast((tween), TimelineLite)._hasPausedChild()))
            {
                return true;
            }
            tween = tween._next;
    ...
    public function getChildren(nested : Bool = true, tweens : Bool = true, timelines : Bool = true, ignoreBeforeTime : Float = -9999999999) : Array<Dynamic>
    {
        ...
        while (tween)
        {
            ...
            else if (Std.is(tween, TweenLite))
            {
                if (tweens)
                {
                    a[cnt++] = tween;
    ...
    /** @private **/
    private function _contains(tween : Animation) : Bool
    {
    ...
    /**
		 ...
		 * @return self (makes chaining easier)
		 */
    public function shiftChildren(amount : Float, adjustLabels : Bool = false, ignoreBeforeTime : Float = 0) : Dynamic
    {
        var tween : Animation = _first;
        while (tween)
        {
        ...
        if (adjustLabels)
        {
            ...
            {
                if (Reflect.field(_labels, p) >= ignoreBeforeTime)
                {
                    Reflect.field(_labels, p) += amount;
                }
            }
        }
        _uncache(true);
    ...
    override public function _kill(vars : Dynamic = null, target : Dynamic = null) : Bool
    {
        ...
        var tweens : Array<Dynamic> = ((target == null)) ? getChildren(true, true, false) : getTweensOf(target);
    ...
    {
        var tweens : Array<Dynamic> = getChildren(false, true, true);
        var i : Int = tweens.length;
        _time = _totalTime = 0;
        while (--i > -1)
        {
    ...
    /** @private **/
    override public function _enabled(enabled : Bool, ignoreTimeline : Bool = false) : Bool
    {
        if (enabled == _gc)
        {
            var tween : Animation = _first;
...
myAnimation.totalDuration( 20 ); //adjusts the timeScale so that myAnimation fits into exactly 20 seconds on its parent timeline
    ...
    override public function totalDuration(value : Float = Math.NaN) : Dynamic
    {
        if (!arguments.length)
        {
            ...
            {
                var max : Float = 0;
                var tween : Animation = _last;
                var prevStart : Float = Infinity;
                var prev : Animation;
                var end : Float;
                while (tween)
                {
...
max -= tween._startTime;
    ...
    /** 
		 ...
		 **/
    public function usesFrames() : Bool
    {
    ...
    /**
		 ...
		 */
    override public function rawTime() : Float
    {
